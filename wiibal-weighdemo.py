#!/usr/bin/python
import sys
sys.path.insert(0, '/home/bin/cwiid/python/build/lib.linux-x86_64-2.7/')
import sys
...
#			print "Warning, %s reading below lower calibration value" % sensor
		...
		if reading < calibration[1]:
			weight += 1650 * (reading - calibration[0]) / (calibration[1] - calibration[0])
		else:
			weight += 1650 * (reading - calibration[1]) / (calibration[2] - calibration[1]) + 1650
